...
task('deploy:ObserverMerkleProvider')
  ...
  .setAction(async function (taskArguments: TaskArguments, { ethers }) {
    ...
    const observerMerkleProvider: ObserverMerkleProvider = <
      ObserverMerkleProvider
    >await observerMerkleProviderFactory.deploy(taskArguments.greeting)
    await observerMerkleProvider.deployed()
    console.log(
      'ObserverMerkleProvider deployed to: ',
      observerMerkleProvider.address
    )
  })
